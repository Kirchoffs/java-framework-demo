...
public class HelloWorldApplication {
    ...
    public Step stepAlpha() {
        return this.stepBuilderFactory.get("step-alpha").tasklet(taskletAlpha()).build();
    }
    ...
    public Tasklet taskletAlpha() {
        return (contribution, chunkContext) -> {
            ...
            if (!parameters.containsKey("gift")) {
                throw new Exception("Exception throwed");
            }
            ExecutionContext executionContext = chunkContext.getStepContext().getStepExecution().getJobExecution().getExecutionContext();
            executionContext.putString("gift", (String) parameters.get("gift"));
            System.out.println("------------");   
